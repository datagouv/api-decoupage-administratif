...
def download_communes_siren():
    ...
    if "Siren" in banatic_df.columns:
        ...
        print(
            f" Saving SIREN mapping to {mapping_file}... (we will reuse it in other scripts)"
        )
    ...
    for encoding in encodings:
        try:
            interco_df = pd.read_csv(
                BytesIO(response.content), encoding=encoding, dtype=str, sep=";"
            )
            print(f"✓ Successfully loaded with encoding: {encoding}")
            break
        except Exception:
...
def download_codes_postaux():
    ...
    try:
        ...
        if os.path.exists(MISSING_CODES_FILE):
            print("  Loading missing postal codes...")
            try:
        ...
        print("  Grouping postal codes by commune...")
        commune_codes = (
            df.groupby("codeCommune")["codePostal"]
...
def download_and_filter_communes():
    ...
    if interco_df is not None and "Siren" in df.columns:
        ...
        print("✓ Merge complete")
        print(
            f"  {df['siren_interco'].notna().sum()} / {len(df)} entities have intercommunalité"
        )
    ...
    if codes_postaux_df is not None:
        ...
        print("✓ Merge complete")
        print(
            f"  {df['codes_postaux'].notna().sum()} / {len(df)} entities have postal codes"
        )
    ...
    if "siren_interco" in communes_df.columns:
        print(
            f"    - With intercommunalité: {communes_df['siren_interco'].notna().sum()}"
        )
    ...
    if interco_df is not None:
        print(
            f"  - Entities with intercommunalité: {df['siren_interco'].notna().sum()} / {len(df)}"
        )
    if codes_postaux_df is not None:
        print(
            f"  - Entities with postal codes: {df['codes_postaux'].notna().sum()} / {len(df)}"
        )
...
def download_interco_members():
    ...
    try:
        ...
        print("  Saving CSV version for faster access...")
...
def resolve_communes_for_interco(
    interco_siren, members_df, interco_df, depth=0, max_depth=5, visited=None
):
    """
...
def enrich_interco_with_communes(interco_df, members_df):
    ...
    if os.path.exists(mapping_file):
        ...
        try:
            ...
            print("  communes_code will not be populated")
    else:
    ...
    for idx, row in interco_df.iterrows():
        ...
        membres_list = (
            direct_members[membre_col].dropna().tolist()
        ...
        interco_df.at[idx, "membres_siren"] = (
            json.dumps(membres_list) if membres_list else None
        )
        interco_df.at[idx, "communes_siren"] = (
            json.dumps(communes_list) if communes_list else None
        )
        interco_df.at[idx, "communes_code"] = (
            json.dumps(communes_code_list) if communes_code_list else None
        )
    ...
    print(
        f"  intercommunalités with communes SIREN: {len(interco_df[interco_df['nb_communes'] > 0])}"
    )
    ...
    print(
        f"  Average communes per intercommunalité: {interco_df['nb_communes'].astype(int).mean():.1f}"
    )
    print(
        f"  Max communes in an intercommunalité: {interco_df['nb_communes'].astype(int).max()}"
    )
...
def download_aom_source(force: bool = False) -> str:
    ...
    print("📥 Téléchargement AOM depuis data.gouv.fr...")
...
def prepare_aom_tables(
    ...
    if len(aom_from_sheet) < before:
        print(
            f"  ⚠️  {before - len(aom_from_sheet)} doublons SIREN AOM ignorés (onglet AOM)"
        )
    ...
    composition_for_names = composition_for_names[
        composition_for_names["siren"].notna()
    ]
    composition_for_names = composition_for_names[
    ...
    composition_df = composition_df.drop_duplicates(
        subset=["siren_commune", "siren_aom"]
    )
...
if __name__ == "__main__":
    ...
    print("\nData downloaded:")
    print(
        "  ✓ Communes, arrondissements, communes déléguées (SIREN, intercommunalités, codes postaux)"
    )
