...
def generate_departements_geojson(communes_file_name, tolerance_label):
    ...
    if not os.path.exists(communes_path):
        print(
            f"⚠️  {communes_path} not found. Run this script without --aggregate first."
        )
        return False

    if not os.path.exists(departements_csv):
        print(
            f"⚠️  {departements_csv} not found. Run 1_download_decoupage_administratif_data.py first."
        )
        return False
    ...
    for idx, dep_row in df_dep.iterrows():
        ...
        communes_dep = gdf_communes[
            gdf_communes["code_insee_du_departement"] == dep_code
        ]
    ...
    geojson = {"type": "FeatureCollection", "features": departements_features}
...
def generate_regions_geojson(communes_file_name, tolerance_label):
    ...
    if not os.path.exists(regions_csv):
        print(
            f"⚠️  {regions_csv} not found. Run 1_download_decoupage_administratif_data.py first."
        )
        return False
    ...
    # Create FeatureCollection
    geojson = {"type": "FeatureCollection", "features": regions_features}
...
def generate_interco_geojson(communes_file_name, tolerance_label):
    ...
    output_path = os.path.join(
        DATA_DIR, f"intercommunalites_{tolerance_label}.geojson.gz"
    )
    ...
    if not os.path.exists(interco_csv):
        print(
            f"⚠️  {interco_csv} not found. Run 1_download_decoupage_administratif_data.py first."
        )
        return False
    ...
    for idx, interco_row in df_interco.iterrows():
        ...
        try:
            ...
            properties = interco_row.drop(
                ["communes_code", "communes_siren", "membres_siren"]
            ).to_dict()
            ...
            if processed % 100 == 0:
                print(
                    f"  {processed}/{len(df_interco) - skipped} intercommunalités processed...",
                    end="\r",
                )
    ...
    # Create FeatureCollection
    geojson = {"type": "FeatureCollection", "features": interco_features}
...
def generate_aom_geojson(communes_file_name, tolerance_label):
    ...
    members_by_aom = (
        df_members.groupby("siren_aom")["siren_commune"].apply(list).to_dict()
    )
...
def generate_interco_by_nature(df_interco, commune_geoms, tolerance_label):
    ...
    for nature in natures:
        ...
        nature_clean = (
            nature.replace("/", "_").replace(" ", "_").replace("'", "").lower()
        )
        output_path = os.path.join(
            DATA_DIR, f"intercommunalites_{nature_clean}_{tolerance_label}.geojson.gz"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate aggregated GeoJSON (departements, regions, intercommunalites)"
    )
    ...
    try:
        ...
        success_dep = all(
            [
                generate_departements_geojson(
                    f"communes_{tolerance_label}.geojson.gz", tolerance_label
                )
        ...
        if success_interco:
            ...
            print(
                f"  ✓ intercommunalites_*.geojson.gz (by nature juridique) for levels {', '.join(SIMPLIFICATION_LEVELS_TITLES)}"
            )
        if success_aom:
            for tolerance_label in SIMPLIFICATION_LEVELS_TITLES:
