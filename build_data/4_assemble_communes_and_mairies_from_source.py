...
def merge_communes():
    infos_for_regions_depts_com = pd.read_csv(
        INFOS_FOR_REGIONS_DEPTS_COM, dtype="string"
    ).map(lambda x: np.nan if x is pd.NaT else x)
    print(f"\n{'=' * 60}")
    print("Load and reformat columns for Admin Express communes")
    print(f"{'=' * 60}")
    communes_admin_express = os.path.join(DATA_DIR, "communes-admin-express.geojson.gz")
    communes_not_from_admin_express = os.path.join(
        SOURCE_DIR, COMMUNES_NOT_IN_ADMIN_EXPRESS
    )
    collectivites_territoriales_admin_express = os.path.join(
        DATA_DIR, "collectivite_territoriale.geojson.gz"
    )
    gdf_communes_admin_express = gpd.read_file("/vsigzip/" + communes_admin_express)
    gdf_communes_admin_express.loc[
        gdf_communes_admin_express["code_insee_du_departement"] == "NR",
        "code_insee_du_departement",
    ] = "975"
    gdf_communes_admin_express.loc[
        gdf_communes_admin_express["code_insee_de_la_region"] == "NR",
        "code_insee_de_la_region",
    ] = "975"
    gdf_communes_admin_express = gdf_communes_admin_express[
        [
            "code_insee",
            "nom_officiel",
            "nom_officiel_en_majuscules",
    ...
    print(f"\n{'=' * 60}")
    print(
        "Load and reformat columns for communes from collectivités\nterritoriales in Admin Express"
    )
    print(f"{'=' * 60}")
    gdf_collectivites_territoriales_admin_express = gpd.read_file(
        "/vsigzip/" + collectivites_territoriales_admin_express
    )
    gdf_collectivites_territoriales_admin_express_977_978 = (
        gdf_collectivites_territoriales_admin_express[
            gdf_collectivites_territoriales_admin_express["code_insee"].isin(
                ["977", "978"]
            )
        ]
    )
    gdf_collectivites_territoriales_admin_express_977_978.loc[
        :,
        [
            "code_insee_du_departement",
            "code_insee_de_la_region",
    ...
    gdf_collectivites_territoriales_admin_express_977_978.loc[
        gdf_collectivites_territoriales_admin_express_977_978["code_insee"] == "977",
        "code_insee",
    ] = "97701"
    gdf_collectivites_territoriales_admin_express_977_978.loc[
        gdf_collectivites_territoriales_admin_express_977_978["code_insee"] == "978",
        "code_insee",
    ] = "97801"
    gdf_collectivites_territoriales_admin_express_977_978 = pd.merge(
        gdf_collectivites_territoriales_admin_express_977_978,
        infos_for_regions_depts_com,
        left_on="code_insee",
        right_on="code_commune",
    )
    gdf_collectivites_territoriales_admin_express_977_978[
        "code_insee_du_departement"
    ] = gdf_collectivites_territoriales_admin_express_977_978["code_collectivite"]
    gdf_collectivites_territoriales_admin_express_977_978["code_insee_de_la_region"] = (
        gdf_collectivites_territoriales_admin_express_977_978["code_collectivite"]
    )
    gdf_collectivites_territoriales_admin_express_977_978 = (
        gdf_collectivites_territoriales_admin_express_977_978.rename(
            columns={"population_y": "population"}
        )
    )
    gdf_collectivites_territoriales_admin_express_977_978["nom_officiel"] = (
        gdf_collectivites_territoriales_admin_express_977_978[
            "nom_officiel"
        ].str.replace("Collectivité de ", "")
    )
    gdf_collectivites_territoriales_admin_express_977_978[
        "nom_officiel_en_majuscules"
    ] = gdf_collectivites_territoriales_admin_express_977_978[
        "nom_officiel_en_majuscules"
    ].str.replace("COLLECTIVITE DE ", "")
    gdf_collectivites_territoriales_admin_express_977_978 = (
        gdf_collectivites_territoriales_admin_express_977_978[
            [
                "code_insee",
    ...
    print(f"\n{'=' * 60}")
    print("Load and reformat columns for communes COM not in\nAdmin Express")
    print(f"{'=' * 60}")
    gdf_communes_not_from_admin_express = gpd.read_file(communes_not_from_admin_express)
    gdf_communes_not_from_admin_express = gdf_communes_not_from_admin_express.rename(
        columns={"nom": "nom_officiel"}
    )
    gdf_communes_not_from_admin_express = pd.merge(
        gdf_communes_not_from_admin_express,
        infos_for_regions_depts_com,
        left_on="code_insee",
        right_on="code_commune",
    )
    gdf_communes_not_from_admin_express.loc[
        :,
        [
            "nom_officiel_en_majuscules",
            "statut",
    ...
    gdf_communes_not_from_admin_express["code_insee_du_departement"] = (
        gdf_communes_not_from_admin_express["code_collectivite"]
    )
    gdf_communes_not_from_admin_express["code_insee_de_la_region"] = (
        gdf_communes_not_from_admin_express["code_collectivite"]
    )
    gdf_communes_not_from_admin_express["nom_officiel_en_majuscules"] = (
        gdf_communes_not_from_admin_express["nom_officiel"]
        .str.normalize("NFKD")
        .str.encode("ascii", errors="ignore")
        .str.decode("utf-8")
        .str.upper()
    )
    gdf_communes_not_from_admin_express = gdf_communes_not_from_admin_express[
        [
            "code_insee",
            "nom_officiel",
            "nom_officiel_en_majuscules",
    ...
    print(f"\n{'=' * 60}")
    print("Concat all communes sources and write to a compressed\ngeojson file")
    print(f"{'=' * 60}")
    gdf = pd.concat(
        [
            gdf_communes_admin_express,
            gdf_communes_not_from_admin_express,
            gdf_collectivites_territoriales_admin_express_977_978,
        ]
    )
    ...
    gdf.loc[
        gdf["code_insee_du_departement"].isin(["971", "972", "973", "974", "976"]),
        "zone",
    ] = "drom"
...
def process_mairies():
    ...
    concatenated = pd.concat(
        [
            gpd.read_file(f"{gpkg}", sql=sql, engine="pyogrio")
            for sql in CHEF_LIEUX.values()
        ]
    )
    ...
    gdf_osm = gpd.read_file(
        f"{SOURCE_DIR}/{COMMUNES_NOT_IN_ADMIN_EXPRESS}", sql=query_osm, engine="pyogrio"
    )
    ...
    all_mairies = (
        pd.concat([concatenated, gdf_osm, gdf_mortes_pour_la_france])
        .rename(columns={"code_insee": "commune"})
